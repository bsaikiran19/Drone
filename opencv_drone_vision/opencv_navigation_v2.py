...
class DroneNavigator(Node):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # V2 PATH PLANNING — A* global path planner
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def follow_waypoints(self) -> Twist:
        ...
        - Uses proportional control (Kp = 1.0) capped at WP_MAX_SPEED (0.30 m/s)
          to produce smooth velocity commands in linear.x and linear.y.
        ...
        """
        WP_MAX_SPEED    = 0.70   # m/s — hard cap on each axis
        WP_ACCEPT_RADIUS = 0.70  # m  — waypoint considered reached inside this radius
        WP_KP           = 1.0    # proportional gain
        ...
        if dist < WP_ACCEPT_RADIUS:
            ...
            # Recompute toward the newly selected waypoint
            wp_x, wp_y = self.path[self.current_waypoint]
            dx = wp_x - self.current_x
            dy = wp_y - self.current_y

        # ── Proportional velocity commands (world frame → body frame) ─────────
        # With angular.z = 0 the drone heading stays constant, so world dx / dy
        # map directly to body linear.x / linear.y.
        vx = float(np.clip(WP_KP * dx, -WP_MAX_SPEED, WP_MAX_SPEED))
        vy = float(np.clip(WP_KP * dy, -WP_MAX_SPEED, WP_MAX_SPEED))
        ...
        self.get_logger().debug(
            ...
            f'dist={dist:.2f}m  vx={vx:.2f}  vy={vy:.2f}')
