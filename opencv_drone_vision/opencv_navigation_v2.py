...
LANDING_RADIUS_THRESHOLD = 95     # px  — stop descending once landing pad radius exceeds this

# ─── V2.3 Part 2: Final Soft Landing constants ────────────────────────────────
FINAL_DESCENT_SPEED = -0.03   # m/s — slower, gentler descent rate during final landing
TOUCHDOWN_RADIUS     = 120    # px  — landing pad radius that signals touchdown
...
class DroneNavigator(Node):
    ...
    def __init__(self):
        ...
        self.touchdown_occurred    = False   # True once TOUCHDOWN_RADIUS is reached
    ...
    def _draw_precision_landing_debug(self, frame):
        ...
        if self.state == STATE_CONTROLLED_DESCENT:
            ...
            cv2.putText(debug, f'Landing Radius : {self.landing_pad_radius} / {LANDING_RADIUS_THRESHOLD}',
            ...
            cv2.putText(debug, ready_str,
                        (10, 252), cv2.FONT_HERSHEY_SIMPLEX, 0.65, ready_col, 2)
        ...
        if self.state in (STATE_FINAL_LANDING, STATE_MISSION_COMPLETE):
            if self.state == STATE_MISSION_COMPLETE:
                ...
                vertical_speed = 0.0 if self.touchdown_occurred else FINAL_DESCENT_SPEED
            ...
            cv2.putText(debug, f'Landing Radius : {self.landing_pad_radius} / {TOUCHDOWN_RADIUS}',
                        (10, 204), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Vertical Speed : {vertical_speed:.2f} m/s',
                        (10, 226), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Touchdown Status : {touchdown_str}',
                        (10, 252), cv2.FONT_HERSHEY_SIMPLEX, 0.65, touchdown_col, 2)
    ...
    def controlled_descent(self) -> tuple:
        ...
        the landing pad's enclosing-circle radius exceeds
        LANDING_RADIUS_THRESHOLD — this does NOT touch down, it only limits
        how far the controlled descent is allowed to go.
        ...
        hover_ready is True once the radius threshold has been reached and
        ...
        # ── Descent limit (Option A — landing pad radius) ──────────────────────
        hover_ready = self.landing_pad_radius > LANDING_RADIUS_THRESHOLD
    ...
    def final_landing(self) -> tuple:
        """
        Produce a Twist for the final soft-landing phase: a slower, gentler
        descent (FINAL_DESCENT_SPEED) than controlled_descent(), while
        continuing to use precision_align() for X/Y correction so the drone
        stays centred over the pad all the way down.

        Touchdown condition (Option A — landing pad radius)
        -----------------------------------------------------
        Touchdown is declared once landing_pad_radius > TOUCHDOWN_RADIUS.
        ...
        touchdown is True the instant TOUCHDOWN_RADIUS is exceeded.
        """
        # ── Safety: landing pad lost → stop descent, hover in place ───────────
        if not self.landing_pad_found:
            return Twist(), False
        ...
        touchdown = self.landing_pad_radius > TOUCHDOWN_RADIUS

        if touchdown:
            # Touchdown — zero every axis immediately, no further descent.
            cmd = Twist()
        else:
            cmd.linear.z = FINAL_DESCENT_SPEED   # slow, gentle descent
    ...
    def control_loop(self):
        ...
        # once landing_pad_radius > LANDING_RADIUS_THRESHOLD. Does NOT land,
        ...
        # touchdown once landing_pad_radius > TOUCHDOWN_RADIUS, immediately
