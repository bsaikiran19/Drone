...
DESCENT_SPEED    = -0.10  # m/s — constant downward speed while descending
...
class DroneNavigator(Node):
    ...
    def __init__(self):
        ...
        self.descent_hover_ready  = False   # True once radius threshold reached

        # ── V2.3 Part 2: Final Soft Landing / Mission Complete state ───────────
        self.touchdown_occurred    = False   # True once TOUCHDOWN_ALTITUDE is reached
    ...
    def _draw_precision_landing_debug(self, frame):
        ...
        # ── V2.3 Part 1: Controlled Descent overlay ─────────────────────────────
        if self.state == STATE_CONTROLLED_DESCENT:
            descent_speed = 0.0 if self.descent_hover_ready else DESCENT_SPEED
            ready_str = 'READY TO LAND' if self.descent_hover_ready else 'DESCENDING'
            ready_col = (0, 255, 0) if self.descent_hover_ready else (0, 165, 255)

            cv2.putText(debug, f'Mission State : {self.state}',
                        (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Descent Speed : {descent_speed:.2f} m/s',
                        (10, 182), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Altitude : {self.altitude:.2f} m / {DESCENT_ALTITUDE:.2f} m',
                        (10, 204), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Hover Ready : {self.descent_hover_ready}',
                        (10, 226), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Landing Phase : DESCENT',
                        (10, 248), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 200, 255), 1)
            cv2.putText(debug, ready_str,
                        (10, 274), cv2.FONT_HERSHEY_SIMPLEX, 0.65, ready_col, 2)

        # ── V2.3 Part 2: Final Landing / Mission Complete overlay ──────────────
        if self.state in (STATE_FINAL_LANDING, STATE_MISSION_COMPLETE):
            if self.state == STATE_MISSION_COMPLETE:
                vertical_speed = 0.0
                touchdown_str, touchdown_col = 'MISSION COMPLETE', (0, 255, 0)
                phase_str = 'MISSION COMPLETE'
            elif self.touchdown_occurred:
                vertical_speed = 0.0
                touchdown_str, touchdown_col = 'LANDING', (0, 165, 255)
                phase_str = 'TOUCHDOWN'
            else:
                # Mirror final_landing()'s altitude-tiered descent speed for display
                if self.altitude > 0.25:
                    ...
                    vertical_speed = -0.02
                touchdown_str, touchdown_col = 'LANDING', (0, 165, 255)
                phase_str = 'FINAL DESCENT'

            cv2.putText(debug, f'Mission State : {self.state}',
                        (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, 'FINAL LANDING',
                        (10, 182), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 200, 255), 1)
            cv2.putText(debug, f'Altitude : {self.altitude:.2f} m / {TOUCHDOWN_ALTITUDE:.2f} m',
                        (10, 204), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Vertical Speed : {vertical_speed:.2f} m/s',
                        (10, 226), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.putText(debug, f'Landing Phase : {phase_str}',
                        (10, 248), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 200, 255), 1)
            cv2.putText(debug, f'Touchdown Status : {touchdown_str}',
                        (10, 274), cv2.FONT_HERSHEY_SIMPLEX, 0.65, touchdown_col, 2)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # V2.3 Part 1 — Controlled descent (vertical descent + continuous
    # horizontal correction via the existing precision_align() controller)
    # ─────────────────────────────────────────────────────────────────────────

    def controlled_descent(self) -> tuple:
        """
        Produce a Twist that descends at a constant rate while continuously
        re-using precision_align() to keep the drone centred over the
        landing pad on X/Y. Descent stops (cmd.linear.z = 0, hover) once
        self.altitude reaches DESCENT_ALTITUDE — this does NOT touch down,
        it only limits how far the controlled descent is allowed to go.

        Altitude is taken from self.altitude, which is already populated by
        odom_callback() from msg.pose.pose.position.z — no new odometry
        subscriber is created here.

        Safety
        ------
        If the landing pad is not currently detected, descent is
        immediately stopped and the drone hovers (zero Twist) — horizontal
        correction and vertical descent both pause until redetected.
        Horizontal speed is bounded by precision_align()'s existing
        PRECISION_MAX_SPEED cap (0.08 m/s); vertical speed is fixed at
        DESCENT_SPEED (-0.10 m/s) and is never exceeded.

        Returns
        -------
        (cmd: Twist, hover_ready: bool)
        hover_ready is True once the altitude threshold has been reached and
        the drone is holding altitude rather than still descending.
        """
        # ── Safety: landing pad lost → stop descent, hover in place ───────────
        if not self.landing_pad_found:
            return Twist(), False

        # Continuous horizontal correction reusing the existing controller —
        # precision_align() is not modified; it is only called again here.
        cmd, _ = self.precision_align()

        # ── Descent limit (altitude-based — odometry self.altitude) ────────────
        hover_ready = self.altitude <= DESCENT_ALTITUDE

        if hover_ready:
            cmd.linear.z = 0.0   # threshold reached — hold altitude, do not land
        else:
            cmd.linear.z = DESCENT_SPEED   # keep descending at the fixed rate

        cmd.angular.z = 0.0   # no rotation, consistent with precision_align()

        return cmd, hover_ready

    # ─────────────────────────────────────────────────────────────────────────
    # V2.3 Part 2 — Final soft landing (slower descent + continuous X/Y hold,
    # reusing precision_align() exactly as controlled_descent() does)
    # ─────────────────────────────────────────────────────────────────────────

    def final_landing(self) -> tuple:
        """
        Produce a Twist for the final soft-landing phase: a gradually
        slowing descent rate (tiered by self.altitude) than
        controlled_descent(), while continuing to use precision_align() for
        X/Y correction so the drone stays centred over the pad all the way
        down.

        Descent speed tiers (smooth landing)
        -------------------------------------
        altitude > 0.25 m              → -0.08 m/s
        0.12 m < altitude <= 0.25 m    → -0.05 m/s
        altitude <= 0.12 m             → -0.02 m/s

        Touchdown condition (altitude-based — odometry self.altitude)
        -----------------------------------------------------------------
        Touchdown is declared once self.altitude <= TOUCHDOWN_ALTITUDE.
        At that instant the caller is expected to immediately zero every
        Twist axis and disable the drone — this function reports the
        touchdown flag so control_loop() can do exactly that on the same
        tick, with no extra descent applied afterward.

        Safety
        ------
        If the landing pad is not currently detected, descent and
        horizontal correction both pause (zero Twist) — identical safety
        behaviour to controlled_descent().

        Returns
        -------
        (cmd: Twist, touchdown: bool)
        touchdown is True the instant TOUCHDOWN_ALTITUDE is reached.
        """
        # ── Safety: landing pad lost → stop descent, hover in place ───────────
        if not self.landing_pad_found:
            return Twist(), False

        # Reuse the existing X/Y controller — no duplicate alignment logic.
        cmd, _ = self.precision_align()

        touchdown = self.altitude <= TOUCHDOWN_ALTITUDE

        if touchdown:
            # Touchdown — zero every axis immediately, no further descent.
            cmd = Twist()
        else:
            # ── Gradually slowing descent rate (altitude-based) ─────────────────
            if self.altitude > 0.25:
                cmd.linear.z = -0.08
            elif self.altitude > FINAL_LANDING_ALTITUDE:
                cmd.linear.z = -0.05
    ...
    def control_loop(self):
        ...
        if self.state == STATE_PRECISION_ALIGN:
            ...
            if not self.landing_pad_found:
                ...
                # ── V2.3 Part 1: hand off to controlled descent once aligned ───
                if self.precision_alignment_complete:
        ...
        # ── V2.3 Part 2: STATE_FINAL_LANDING — slow descent + continuous X/Y
        # correction via final_landing() (reuses precision_align()). Declares
        # touchdown once self.altitude <= TOUCHDOWN_ALTITUDE, immediately
        # zeroes all velocity, disables the drone once, then hands off to
        # STATE_MISSION_COMPLETE.
        if self.state == STATE_FINAL_LANDING:

            # Landing pad lost during final landing → stop descent and hover;
            # resume once redetected (same safety behaviour as descent).
            cmd, touchdown = self.final_landing()
            self.touchdown_occurred = touchdown

            if touchdown:
                # ── Immediate zero-velocity Twist on touchdown ──────────────────
                cmd = Twist()
                cmd.linear.x  = 0.0
                cmd.linear.y  = 0.0
                cmd.linear.z  = 0.0
                ...
                # ── Disable the drone once (publish False to /drone/enable) ─────
                enable_msg = Bool()
